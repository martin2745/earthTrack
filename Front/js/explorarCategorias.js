...
function explorarCategorias() {
  ...
  function construyeTarjetaProceso(proceso){
    ...
    proceso.parametros.forEach(parametro => {
          ...
          '<input type="text" maxlength="200" placeholder="'+parametro.unidad+'" name="" id="input_'+parametro.id_parametro+'" class="" onblur="javascript:if(comprobarParametroNumerico('+"'input_"+parametro.id_parametro+"',"+"'errorFormatoParametro"+parametro.id_parametro+"','parametroFormula')){calcularProceso("+"'"+proceso.formula+"'"+")}else{resetTotal()}"+'"'+'/>'+
    ...
    tarjetaFooter= '</br><p class="card-text description" id="label_total_formula">Total</p>'+
                "</div>"+
                '<div class="card-footer">' +
  ...
  function construyeTarjetaProcesoYaCalculado(proceso){
          ...
          '<div class="col-md-4 col-lg-6 col-xl-6 mb-4">' +
            ...
            '<form name="formProceso" id="formProceso" action="javascript:navegarMisProcesos()" onsubmit="">'+
              '<img src="images/rol.png" class="card-img-top" alt="Noticias">' +
                '<div class="card-body-news">' +
                  '<h4 class="card-title">'+proceso.nombre_proceso+'</h4>' +
                  '<p class="card-text description PROCESO_CALCULADO">'+proceso.descripcion_proceso+'</p>'+
                  '<div class="card-text" style="display: flex;">'+
                  '</div>';
    ...
    tarjetaFooter= 
                "</div>"+
                '<div class="card-footer">' +
                    '<button type="submit" name="btnCategorias'+proceso.id_proceso+'"  class="btnCategorias'+proceso.id_proceso+' tooltip3" onClick="" id="btnCategorias'+proceso.id_proceso+'"">'+
                      '<img class="iconoEntrar iconEntrar" src="images/enterLogin.png"/>'+
  ...
  async function buscarProceso(id_categoria){
    ...
    await buscarProcesoAjaxPromesa()
      .then((res) => {
        ...
        console.log(procesosCalculados);
