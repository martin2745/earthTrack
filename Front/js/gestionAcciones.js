...
function guardarParametrosBusqueda(criterios) {
  setCookie("id_accion", criterios.id_accion);
  setCookie("nombre_accion", criterios.nombre_accion);
  setCookie("descripcion_accion", criterios.descripcion_accion);
}
...
function busquedaVacia() {
  ...
  if (
    getCookie("id_accion") != "" ||
    getCookie("nombre_accion") != "" ||
    getCookie("descripcion_accion") != ""
  ) {
    toret = false;
  }
  return toret;
...
function informacionBusqueda(form) {
  insertacampoParaBusqueda(form, "id_accion", getCookie("id_accion"));
  insertacampoParaBusqueda(form, "nombre_accion", getCookie("nombre_accion"));
...
function cargarInformacionBusqueda() {
  if (getCookie("id_accion") != "") {
    document.getElementById("id_accion_BUSQUEDA").value =
      getCookie("id_accion");
  } else {
    document.getElementById("id_accion_BUSQUEDA").value = "";
  }

  if (getCookie("nombre_accion") != "") {
    document.getElementById("nombre_accion_BUSQUEDA").value =
      getCookie("nombre_accion");
  } else {
    document.getElementById("nombre_accion_BUSQUEDA").value = "";
  }

  if (getCookie("descripcion_accion") != "") {
    document.getElementById("descripcion_accion_BUSQUEDA").value =
      getCookie("descripcion_accion");
  } else {
    document.getElementById("descripcion_accion_BUSQUEDA").value = "";
  }
...
/*Función que establece que acciones tenemos para una accion determinada.
 En el caso de accion necesitamos saber si existe el permiso para insertar por ejemplo,
...
async function accionesPermitidas() {
  ...
  que acciones tenemos para una accion.*/
  crearformoculto(
  ...
  /*Insertamos el campo nombre_accion con la accion
  ...
  await accionesPermitidasAjaxPromesa()
    .then((res) => {
      /*Guardamos en una cookie las acciones de la accion.*/
...
function accionesInsertarBuscar() {
  ...
  $("#accionesCabecera").append(permisos);
...
async function getListadoEntidades() {
  var idioma = getCookie("lang");
  addActionControler(document.formularioListado, "buscar", "accion");
...
function addEstructura() {
  ...
  $("#input_accion_id").attr("style", "display: none");
}
...
function verEnDetalleEstructura() {
  ...
  $("#label_accion_id").attr("style", "display: none");
  $("#input_accion_id").attr("style", "display: none");
}
...
function activarCamposInLineBlock() {
  $("#input_accion_id").attr("style", "display: inline-block");
  $("#input_accion_nombre").attr("style", "display: inline-block");
  $("#input_accion_descripcion").attr("style", "display: inline-block");
...
function activarCamposBlock() {
  $("#input_accion_id").attr("style", "display: block");
  $("#input_accion_nombre").attr("style", "display: block");
  $("#input_accion_descripcion").attr("style", "display: block");
...
function mostrarObligatorios() {
  $("#obligatorio_accion_nombre").attr("style", "display: inline-block");
  $("#obligatorio_accion_description").attr("style", "display: inline-block");
}

function ocultarObligatorios() {
  $("#obligatorio_accion_nombre").attr("style", "display: none");
  $("#obligatorio_accion_description").attr("style", "display: none");
}

function mostrarLabels() {
  $("#label_accion_id").attr("style", "display: block");
  $("#label_accion_nombre").attr("style", "display: block");
  $("#label_accion_description").attr("style", "display: block");
}

function ocultarLabels() {
  $("#label_accion_id").attr("style", "display: none");
  $("#label_accion_nombre").attr("style", "display: none");
  $("#label_accion_description").attr("style", "display: none");
}
...
function showAdd() {
  ...
  cambiarFormulario(
    ...
    "return comprobarAddAccion();"
  );
  cambiarOnBlurCampos(
    "",
    "return comprobarNombreAccion('input_accion_nombre', 'errorFormatoNombre', 'nombreAccion')",
    "return comprobarDescripcionAccion('input_accion_descripcion', 'errorFormatoDescripcion', 'descripcionAccion')"
  );
...
function showSearch() {
  ...
  cambiarFormulario(
    ...
    "return comprobarSearchAccion();"
  );
  cambiarOnBlurCampos(
    "",
    "return comprobarNombreAccionSearch('input_accion_nombre', 'errorFormatoNombre', 'nombreAccion')",
    "return comprobarDescripcionAccionSearch('input_accion_descripcion', 'errorFormatoDescripcion', 'descripcionAccion')"
  );
  cambiarIcono(
    "images/search.png",
    "ICONO_SEARCH",
...
function showEditar(id, nombre, descripcion) {
  var idioma = getCookie("lang");
  var campos = ["input_accion_id", "input_accion_nombre"];
  editEstructura();
  cambiarFormulario(
    "editForm",
    "javascript:editEntidad();",
    "return comprobarEditAccion();"
  );
  cambiarOnBlurCampos(
    "",
    "return comprobarNombreAccion('input_accion_nombre', 'errorFormatoNombre', 'nombreAccion')",
...
function rellenarFormulario(id, nombre, descricion) {
  $("#input_accion_id").val(id);
  $("#input_accion_nombre").val(nombre);
  $("#input_accion_descripcion").val(descricion);
}
...
function cambiarOnBlurCampos(onBlurId, onBlurNombre, onBlurdescripcion) {
  if (onBlurId != "") {
    $("#input_accion_id").attr("onblur", onBlurId);
  }

  if (onBlurNombre != "") {
    $("#input_accion_nombre").attr("onblur", onBlurNombre);
  }

  if (onBlurdescripcion != "") {
    $("#input_accion_descripcion").attr("onblur", onBlurdescripcion);
  }
}
...
function addEntidadAjaxPromesa() {
  var token = getCookie("token");
  addActionControler(document.formularioGenerico, "insertar", "accion");
  if (token == null) {
    errorAutenticado("ACCESO_DENEGADO");
  } else {
    return new Promise(function (resolve, reject) {
      $.ajax({
        ...
        .done((res) => {
          if (res.code != "ACCION_INSERTAR_OK") {
            reject(res);
          }
          resolve(res);
        })
...
function searchEntidadAjaxPromesa() {
  var token = getCookie("token");
  addActionControler(document.formularioGenerico, "buscar", "accion");
  if (token == null) {
    errorAutenticado("ACCESO_DENEGADO");
  } else {
    return new Promise(function (resolve, reject) {
...
/**Función para cerrar la ventana de detalle de accion*/
...
function editEntidadAjaxPromesa() {
  var token = getCookie("token");
  addActionControler(document.formularioGenerico, "editar", "accion");
  habilitaCampos([
    "input_accion_id",
    "input_accion_nombre",
    "input_accion_descripcion",
...
function deleteEntidadAjaxPromesa() {
  var token = getCookie("token");
  addActionControler(document.formularioGenerico, "borrar", "accion");
  habilitaCampos(["input_accion_id"]);
  if (token == null) {
    errorAutenticado("ACCESO_DENEGADO");
  } else {
    return new Promise(function (resolve, reject) {
