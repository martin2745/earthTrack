...
function explorarProcesos() {
  ...
  function construyeTarjetaProceso(proceso){
    ...
    tarjeta =
          '<div class="col-md-4 col-lg-6 col-xl-6 mb-4">' +
            '<div class="card">' +
            '<form name="formCategoria'+proceso.id_proceso+'" id="formCategoria'+proceso.id_proceso+'" action="javascript:if(comprobarParametros('+"'"+proceso.id_proceso+"'"+')){guardarProceso('+"'"+nombre_formulario+"'"+')}" onsubmit="">'+
              '<img src="images/rol.png" class="card-img-top" alt="Noticias">' +
                '<div class="card-body-news">' +
                  ...
                  '<strong>'+proceso.categoria.nombre_categoria+'</strong>'+
                  '</div>';
    ...
    proceso.parametros.forEach(parametro => {
          ...
          '<input type="text" maxlength="200" placeholder="'+parametro.unidad+'" name="" id="input_'+parametro.id_parametro+'" value="'+parametro.valor+'" class="" onblur="javascript:if(comprobarParametroNumerico('+"'input_"+parametro.id_parametro+"',"+"'errorFormatoParametro"+parametro.id_parametro+"','parametroFormula')){calcularProceso("+"'"+proceso.formula+"',"+"'"+proceso.id_proceso+"')}else{resetTotal("+"'"+proceso.id_proceso+"')}"+'"'+'/>'+
    ...
    tarjetaFooter= '</br><p class="card-text description" id="label_total_formula'+proceso.id_proceso+'">Total</p>'+
                "</div>"+
                '<div class="card-footer">' +
                  //  '<form name="formCategoria'+proceso.id_proceso+'" id="formCategoria'+proceso.id_proceso+'" action="javascript:calcularProceso('+"'"+nombre_formulario+"'"+')" onsubmit="">'+
                    '<input type="hidden" id="'+proceso.id_proceso+'" name="id_proceso" value="'+proceso.id_proceso+'">'+
                    '<input type="hidden" id="valor_parametros_'+proceso.id_proceso+'" name="parametros" value="">'+
                    '<input type="hidden" id="usuario_'+proceso.id_proceso+'" name="usuario" value="'+usuario+'">'+
                    '<button type="submit" name="btnCategorias'+proceso.id_proceso+'"  class="btnCategorias'+proceso.id_proceso+' tooltip3" id="btnCategorias'+proceso.id_proceso+'"">'+
                      ...
                      '</button>'+"</form>"+
                     '<form  id="formDeleteProceso'+proceso.id_proceso+'" action="javascript:deleteProceso('+"'"+nombre_formulario_delete+"'"+')">' +
                     '<input type="hidden" id="delete'+proceso.id_proceso+'" name="id_proceso" value="'+proceso.id_proceso+'">'+
                    '<input type="hidden" id="delete_usuario_'+proceso.id_proceso+'" name="usuario" value="'+usuario+'">'+
                    '<button type="submit" name="btnDeleteCategorias'+proceso.id_proceso+'"  class="btnCategorias'+proceso.id_proceso+' tooltip3" id="btnCategorias'+proceso.id_proceso+'"">'+
  ...
  function comprobarParametros(id_proceso){
      ...
      parametros.forEach(element=>{
        toret=comprobarParametroNumerico("input_"+element,"errorFormatoParametro"+element,"parametroFormula");
  ...
  function calcularProceso(formula,id_proceso){
      ...
      console.log($("#valor_parametros_"+id_proceso).val());
      ...
      caracteres.forEach(element => {
        if(dentroDeParametro){
          ...
          if(element=="}"){
            dentroDeParametro=false;
            formulaProcesada=formulaProcesada.replace(cadenaAReemplazar,valores.shift());
            cadenaAReemplazar="";
          }
        }
        if(element=="{"){
...
function cargarProcesosCalculadosAjaxPromesa(){
  var token = getCookie("token");
  var idioma = getCookie("lang");
  addActionControler(document.formCargaProcesos, "devolverProcesos", "proceso_usuario");
  ...
  if (token == null) {
    ...
    return new Promise(function (resolve, reject) {
      $.ajax({
        ...
        .done((res) => {
          if (res.code!="PROCESO_USUARIO_DEVOLVER_PROCESOS_OK" && res.code != "RECORDSET_DATOS" && res.code != "RECORDSET_VACIO") {    
            reject(res);
          }
          resolve(res);
        })
