...
function explorarProcesos() {
  ...
  function construyeTarjetaProceso(proceso){
    ...
    proceso.parametros.forEach(parametro => {
          ...
          '<input type="text" maxlength="200" placeholder="'+parametro.unidad+'" name="" id="input_'+parametro.id_parametro+'" class="" onblur="javascript:if(comprobarParametroNumerico('+"'input_"+parametro.id_parametro+"',"+"'errorFormatoParametro"+parametro.id_parametro+"','parametroFormula')){calcularProceso("+"'"+proceso.formula+"',"+"'"+proceso.id_proceso+"')}else{resetTotal("+"'"+proceso.id_proceso+"')}"+'"'+'/>'+
    ...
    async function procesosSistema() {
      ...
      await procesosSistemaAjaxPromesa()
        .then((res) => {
          ...
          console.log(procesosYaCalculados);
